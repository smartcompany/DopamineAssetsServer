...
create table if not exists public.dopamine_hot_mover_discussion_config (
  ...
  push_title_ja text not null default '🔥 今アツい討論',
  push_title_zh text not null default '🔥 正在热议',
  ...
  push_body_template_ja text not null default $hmdc_body_ja$💬 {name} が{direction}（{pct}）・コミュニティが大盛り上がり 👀 今すぐチェック！$hmdc_body_ja$,
  push_body_template_zh text not null default $hmdc_body_zh$💬 {name} {direction}（{pct}）· 社区热度爆表 👀 现在就来看看！$hmdc_body_zh$,
  updated_at timestamptz not null default now()
);
...
-- ---------------------------------------------------------------------------
-- 12b) 관심 자산 일별 점수 히스토리 (interest-assets-daily 크론)
-- ---------------------------------------------------------------------------
...
comment on table public.dopamine_interest_asset_scores is
  '관심 자산 일별 점수·순위. API(service_role)만 접근. score_history: [{date,score,rank}].';
